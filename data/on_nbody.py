...
class NBodyDataset:
    ...
    def load(self):
        ...
        loc = np.load(dataroot + "loc_" + self.sufix + ".npy")
        # vel = np.load('n_body_system/dataset/vel_' + self.sufix + '.npy')
        vel = np.load(dataroot + "vel_" + self.sufix + ".npy")
        # edges = np.load('n_body_system/dataset/edges_' + self.sufix + '.npy')
        edges = np.load(dataroot + "edges_" + self.sufix + ".npy")

        # charges = np.load('n_body_system/dataset/charges_' + self.sufix + '.npy')
        charges = np.load(dataroot + "charges_" + self.sufix + ".npy")
        print("LOC SHAPE:", loc[0].shape)
    ...
    def __getitem__(self, i):
        ...
        # print("LOC SHAPE:", loc[frame_0].shape)
